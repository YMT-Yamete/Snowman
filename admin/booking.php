<?php 
    ...
    if (isset($_POST['finish'])) {
        ...
        $update = "UPDATE Booking SET status = 'Finished' WHERE BookingID = '$id'";
        $updateQuery = mysqli_query($connect, $update);
        $delete = "DELETE FROM schedule WHERE BookingID = '$id'";
        $deleteQuery = mysqli_query($connect, $delete);

        if ($updateQuery && $deleteQuery) {
            echo "<script>alert('Finished servicing. You can still view the data in history.')</script>";
            echo "<script>
                    if ( window.history.replaceState ) {
                        window.history.replaceState( null, null, window.location.href );
                    }
    ...
    $select = "SELECT * FROM booking WHERE status = 'Pending'";
...
<body> 
    <header class="header">
        <nav class="navbar">
            ...
            <ul class="nav-menu">
                <li class="nav-item">
                    <a href="booking.php" class="activeNav">Booking</a>
                </li>
                <li class="nav-item">
    ...
    <p class="fieldHeader">Booking List</p><br>
    ...
    <footer class="footer-distributed" style="<?php print ($count>2)?'margin-top: 0px':'position: fixed'; ?>">
